...
1. **Main Figures (1-4)**: workflow, benchmark validation, FHL CCI ranking and cell-composition profile
2. **Supplementary Figures (S1-S11)**: cross-disease heatmap, sepsis validation, MAS pseudobulk validation, sensitivity analyses, extended benchmark, parameter sensitivity, CADE-ILR rank stability, pathway activity, WGCNA, target-gene signatures and empirical comparator/runtime benchmark
...
5. **Software**: `CADE_NARGeB_submission_upload_2026-06-14_final_no_github_release.zip` (MIT license; CADE version 1.1.0)
...
| Figure S8 | `Supplementary_Figure_S8_Pathway_Analysis.tif` | GSVA and pathway-supporting analyses: ferroptosis/iron GSVA pathways, multi-cell-death comparison and pathway-level delta versus significance |
| Figure S9 | `Supplementary_Figure_S9_WGCNA_Modules.tif` | Exploratory WGCNA module correlations, module-size/r/FDR bubble plot and hub-gene network panels |
| Figure S10 | `Supplementary_Figure_S10_TF_TargetGene_Signature.tif` | TF target-gene signature scoring, not TF activity: per-target delta, aggregate target delta and scoring scope |
| Figure S11 | `Supplementary_Figure_S11_Comparator_Runtime_Benchmark.tif` | Empirical comparator and runtime/scalability benchmark: true-DE AUROC, composition-driven top-100 fraction, CCI stable-vs-composition ranking, and runtime scaling |
