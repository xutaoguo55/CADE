...
for(seed in c(42, 123, 456, 789, 2026, 31415, 271828, 141421, 618034, 999999)) {
  ...
  all_results[[as.character(seed)]] <- data.frame(Seed=seed, Gene=null_genes, CCI=as.numeric(null_cci))
}

combined <- do.call(rbind, all_results)
write.csv(combined, file.path(PR, "submission_upload_nargab_2026-06-04/analysis_output/CADE/Table_S_NullControl_10Seeds.csv"), row.names=FALSE)

suppressPackageStartupMessages(library(dplyr))
summary_stats <- combined %>% group_by(Seed) %>% summarise(
  N=n(), Mean_CCI=mean(CCI, na.rm=TRUE), Median_CCI=median(CCI, na.rm=TRUE),
  SD_CCI=sd(CCI, na.rm=TRUE),
  Pct_CCI_gt_0_5=mean(CCI>0.5, na.rm=TRUE)*100,
  Pct_CCI_lt_0_2=mean(CCI<0.2, na.rm=TRUE)*100
)
print(summary_stats)
write.csv(summary_stats, file.path(PR, "submission_upload_nargab_2026-06-04/analysis_output/CADE/Table_S_NullControl_10Seeds_Summary.csv"), row.names=FALSE)
cat("Done\n")
